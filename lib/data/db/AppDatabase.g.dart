    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Task` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `description` TEXT, `time` TEXT, `status` INTEGER)');
...
class _$TaskDao extends TaskDao {
  ...
  @override
  Future<List<Task>> getTask(bool status) async {
    return _queryAdapter.queryList('SELECT * FROM Task WHERE status = ?',
        arguments: <dynamic>[status == null ? null : (status ? 1 : 0)],
        mapper: _taskMapper);
  }
